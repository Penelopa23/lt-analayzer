...
public class CreateHtmlReportOracle implements CreateReport {

    private static List<TimedReport> reportList = new LinkedList<>();
    private static File fileOracle;
    ...
    private final AnalyzerOracle analyzerOracle;
    private final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private final SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd");

    public static List<TimedReport> getTimedReportedOracle() { return reportList; }
    public static File fileOracle() { return fileOracle; }

    @Override
    public void createReport(Date from, Date to, String path) {
        reportList = Arrays.stream(OracleModules.values())
                .map(module -> analyzerOracle.executeSendReport(from, to, module))
