...
@Slf4j
@Component
public class AnalyzerOracle {
    private final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    private final SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd");
    private Gson gson = new Gson();
    private Date from;
    private Date to;
    private OracleModules modules;
    private Connection connection;

    @PostConstruct
    private void init() {
        connection = Connections.ORACLE.DB();
    }

    private List<String> getSqlR() {
        List<String> sqlR = Arrays.asList(
                SQLRequestsOracle.REQUEST.getRequest(),
                SQLRequestsOracle.RESPONSE.getRequest(),
                SQLRequestsOracle.SUCCESS.getRequest(),
                SQLRequestsOracle.WORKFLOWBROKEN.getRequest(),
                SQLRequestsOracle.ERROR.getRequest()
        );
        return sqlR;
    }
    ...
    private void executeWFB(TimedReport timedReport) {
        ...
        Gson gson = new Gson();
        sql = MessageFormat.format(
                SQLRequestsOracle.WORKFLOWBROKENWITHTEXT.getRequest(),
                modules.getTablePrefix(),
                sdf.format(from),
                sdf.format(to),
                modules.getType().getReceiveRequests()
    ...
    @SneakyThrows
    public TimedReport executeSendReport(Date from, Date to, OracleModules module) {
